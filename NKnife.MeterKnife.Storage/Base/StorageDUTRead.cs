...
namespace NKnife.MeterKnife.Storage.Base
{
    public class StorageDUTRead<T> : IStorageDUTRead<T>
    {
        ...
        public virtual async Task<IEnumerable<T>> PageAsync((Project, DUT) dut, int pageNumber, int pageSize, SortDirection direction)
        {
        ...
        /// <param name="dut">指定的被测试物</param>
        /// <returns></returns>
        public virtual async Task<T> FindOneByIdAsync((Project, DUT) dut, DateTime id)
        {
            var conn = _storageManager.OpenConnection(dut.Item1);
        ...
        public virtual async Task<bool> ExistAsync((Project, DUT) dut, DateTime id)
        {
            var conn = _storageManager.OpenConnection(dut.Item1);
        ...
        /// <param name="dut">指定的被测试物</param>
        /// <returns>数量</returns>
        public virtual async Task<long> CountAsync((Project, DUT) dut)
        {
            var conn = _storageManager.OpenConnection(dut.Item1);
        ...
        /// <param name="dut">指定的被测试物</param>
        /// <returns></returns>
        public virtual async Task<IEnumerable<T>> FindAllAsync((Project, DUT) dut)
        {
            var conn = _storageManager.OpenConnection(dut.Item1);
