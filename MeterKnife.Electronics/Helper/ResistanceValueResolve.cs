...
using MeterKnife.Electronics.Exceptions;
...
namespace MeterKnife.Electronics.Helper
{
    ...
    public class ResistanceValueResolve
    {
        /// <summary>已存在的电阻的集合
        /// </summary>
        public NKnife.Electronics.Collections.Resistances Existing { get; set; }
        ...
        public double TryGetResolveResult(out NKnife.Electronics.Collections.Resistances[] results, ResistanceValueResolveModel model, double target, int begin = 0)
        {
            ...
            var resultlist = new List<NKnife.Electronics.Collections.Resistances>();
            ...
            {
                var result = new NKnife.Electronics.Collections.Resistances();
        ...
        /// <param name="begin">已存在的集合的起始位置，默认从0的位置开始搜索</param>
        /// <returns>分解剩余的阻值余数</returns>
        protected double TryGetSeriesBigValuePriorityResult(ref NKnife.Electronics.Collections.Resistances result, double target, int begin)
        {
            ...
            for (int i = begin; i < Existing.Count; i++)
            {
                ...
                if (target < Existing[i].Value) //如果目标值小于集合当前值，继续向数列下一个进行寻找合适值
                    continue;
                NKnife.Electronics.Resistance res = Existing[i];
                if (target/res.Value > 1)
                {
                    var count = (int) (target/res.Value);
                    for (int j = 0; j < count; j++)
                    {
                        result.Add((NKnife.Electronics.Resistance) res.Clone());
                        target -= res.Value;
                    }
                }
                //余数进行下一步的递归分解
        ...
        private NKnife.Electronics.Collections.Resistances _SmallPrioritResult;
        ...
        /// <returns>分解剩余的阻值余数</returns>
        protected double TryGetSeriesSmallValuePriorityResult(ref NKnife.Electronics.Collections.Resistances result, double target, int end)
        {
            if (end < 0 || end >= Existing.Count)
                end = Existing.Count - 1;

            if (result.Count == 0 && (_SmallPrioritResult == null || _SmallPrioritResult.Count > 0))
                _SmallPrioritResult = new NKnife.Electronics.Collections.Resistances();
            ...
            for (int i = end; i >= 0; i--)//反向循环
            {
                ...
                if (count > 1 && count > _SmallPrioritResult.Count)
                {
                    result.Clear();
                    for (int j = 0; j < count; j++)
                        result.Add((NKnife.Electronics.Resistance) Existing[i].Clone());
                }
                var remainder = target%resValue;
                if (remainder>0)
                {
                    var bigResult = new NKnife.Electronics.Collections.Resistances();
